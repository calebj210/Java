 ...
 */
public class HeapAPQTest {

	HeapAPQ<Integer, String> heappq;
	
	@Before
	public void setUp() throws Exception {
		heappq = new HeapAPQ<Integer, String>();
	}
	...
	public void test1000entries() {
		...
		for (int i=1;i<=1000;i++) {
			assertEquals((Object)i, heappq.removeMin().getKey());
		}
	}
	...
	@Test
	public void test1000entriesReplaceKey() {
		Entry<Integer, String>[] e= (Entry<Integer,String>[])new Entry[1001];
		for (int i=1;i<=1000;i++) {
			e[i]=heappq.insert(i, i+"");
		}
		for (int i=1;i<=1000;i++) {
			heappq.replaceKey(e[i], e[i].getKey()* -1);
		}
		for (int i=-1000;i<=-1;i++) {
			assertEquals((Object)(i), heappq.removeMin().getKey());
		}
